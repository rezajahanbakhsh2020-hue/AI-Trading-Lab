import pytest

from src.evaluation.report_alert_threshold_range_stability_consistency_quality_score_trend_summary import (
    build_quality_score_trend_summary,
)


def test_trend_summary_returns_final_scores_and_trends() -> None:
    history = [
        {"alert_count": 5},
        {"alert_count": 3},
        {"alert_count": 2},
    ]

    result = build_quality_score_trend_summary(
        history,
        lower_bound=2,
        upper_bound=4,
    )

    assert result["snapshot_count"] == 3
    assert result["lower_bound"] == 2
    assert result["upper_bound"] == 4

    assert result["quality_score"] == pytest.approx(2 / 3)
    assert result["stability_score"] == pytest.approx(2 / 3)
    assert result["consistency_score"] == pytest.approx(1.0)

    assert result["quality_trend"] == "up"
    assert result["stability_trend"] == "up"
    assert result["consistency_trend"] == "flat"

    assert result["quality_change"] == pytest.approx(1.0)
    assert result["stability_change"] == pytest.approx(2 / 3)
    assert result["consistency_change"] == pytest.approx(0.0)


def test_trend_summary_reports_declining_quality() -> None:
    history = [
        {"alert_count": 2},
        {"alert_count": 5},
        {"alert_count": 6},
    ]

    result = build_quality_score_trend_summary(
        history,
        lower_bound=2,
        upper_bound=4,
    )

    assert result["quality_score"] == pytest.approx(1 / 3)
    assert result["stability_score"] == pytest.approx(1 / 3)
    assert result["consistency_score"] == pytest.approx(1.0)

    assert result["quality_trend"] == "down"
    assert result["stability_trend"] == "down"
    assert result["consistency_trend"] == "flat"

    assert result["quality_change"] == pytest.approx(-1.0)
    assert result["stability_change"] == pytest.approx(-2 / 3)
    assert result["consistency_change"] == pytest.approx(0.0)


def test_trend_summary_handles_single_snapshot() -> None:
    result = build_quality_score_trend_summary(
        [{"alert_count": 3}],
        lower_bound=2,
        upper_bound=4,
    )

    assert result["snapshot_count"] == 1

    assert result["quality_score"] == pytest.approx(1.0)
    assert result["stability_score"] == pytest.approx(1.0)
    assert result["consistency_score"] == pytest.approx(1.0)

    assert result["quality_trend"] == "flat"
    assert result["stability_trend"] == "flat"
    assert result["consistency_trend"] == "flat"

    assert result["quality_change"] == 0.0
    assert result["stability_change"] == 0.0
    assert result["consistency_change"] == 0.0


def test_trend_summary_handles_empty_history() -> None:
    result = build_quality_score_trend_summary(
        [],
        lower_bound=2,
        upper_bound=4,
    )

    assert result["snapshot_count"] == 0

    assert result["quality_score"] == 0.0
    assert result["stability_score"] == 0.0
    assert result["consistency_score"] == 0.0

    assert result["quality_trend"] == "flat"
    assert result["stability_trend"] == "flat"
    assert result["consistency_trend"] == "flat"

    assert result["quality_change"] == 0.0
    assert result["stability_change"] == 0.0
    assert result["consistency_change"] == 0.0


def test_trend_summary_contains_expected_keys() -> None:
    result = build_quality_score_trend_summary(
        [{"alert_count": 3}],
        lower_bound=2,
        upper_bound=4,
    )

    expected_keys = {
        "snapshot_count",
        "lower_bound",
        "upper_bound",
        "quality_score",
        "stability_score",
        "consistency_score",
        "quality_trend",
        "stability_trend",
        "consistency_trend",
        "quality_change",
        "stability_change",
        "consistency_change",
    }

    assert set(result) == expected_keys
