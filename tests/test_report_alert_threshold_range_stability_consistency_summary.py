...
from src.evaluation.report_alert_threshold_range_stability_consistency_summary import (
    build_stability_consistency_summary,
...
def test_build_summary():
    ...
    summary = build_stability_consistency_summary(
        history,
        1,
        3,
    )
...
def test_invalid_lower_bound_type_raises():
    with pytest.raises(TypeError):
        calculate_stable_count(
            [{"alert_count": 2}],
            1.0,
...
def test_invalid_upper_bound_type_raises():
    with pytest.raises(TypeError):
        calculate_stable_count(
            [{"alert_count": 2}],
            1,
