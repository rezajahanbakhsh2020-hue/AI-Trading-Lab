...
from src.evaluation.report_alert_threshold_range_quality_stability_score import (
    build_quality_stability_score_summary,
    calculate_quality_ratio,
    calculate_quality_stability_score,
    calculate_quality_stability_score_percentage,
    calculate_stability_ratio,
)


def test_quality_ratio_for_all_values_inside_range():
    history = [
        {"alert_count": 2},
        {"alert_count": 3},
        {"alert_count": 4},
...
def test_quality_ratio_for_partially_inside_range():
    history = [
        {"alert_count": 1},
        {"alert_count": 2},
        {"alert_count": 5},
...
def test_stability_ratio_when_all_states_match():
    history = [
        {"alert_count": 2},
        {"alert_count": 3},
        {"alert_count": 4},
...
def test_stability_ratio_when_every_transition_changes_state():
    history = [
        {"alert_count": 3},
        {"alert_count": 6},
        {"alert_count": 2},
...
def test_quality_stability_score_is_average_of_quality_and_stability():
    history = [
        {"alert_count": 2},
        {"alert_count": 5},
        {"alert_count": 3},
...
def test_quality_stability_score_percentage():
    history = [
        {"alert_count": 2},
        {"alert_count": 3},
        {"alert_count": 4},
    ...
    assert calculate_quality_stability_score_percentage(
        history,
        2,
        4,
    ) == 100.0
...
def test_build_summary():
    ...
    expected_score = ((2 / 3) + 0.5) / 2

    assert summary["snapshot_count"] == 3
    assert summary["lower_bound"] == 2
    assert summary["upper_bound"] == 4
    assert summary["quality_ratio"] == pytest.approx(2 / 3)
    assert summary["stability_ratio"] == pytest.approx(0.5)
    assert summary["quality_stability_score"] == pytest.approx(
        expected_score
    )
    assert summary["quality_stability_score_percentage"] == pytest.approx(
