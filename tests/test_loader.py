...
def test_load_csv_accepts_string_path(tmp_path):
    csv_file = tmp_path / "xauusd.csv"
...
def test_load_csv_preserves_volume_when_present(tmp_path):
    csv_file = tmp_path / "xauusd.csv"

    csv_file.write_text(
        "timestamp,open,high,low,close,volume\n"
        "2026-01-01 00:00:00,2650,2660,2640,2655,1000\n"
        "2026-01-01 01:00:00,2655,2670,2650,2665,1200\n"
    )

    result = load_csv(csv_file)

    assert "volume" in result.columns
    assert list(result["volume"]) == [1000, 1200]
