from __future__ import annotations

from collections.abc import Mapping
from typing import Any

from src.evaluation.report_alert_threshold_range_stability_consistency_quality_score_trend_detail_summary_extrema_comparison_detail_summary_extrema_report_summary_detail import (
    build_quality_score_trend_detail_summary_extrema_comparison_detail_summary_extrema_report_summary_detail,
)


def build_quality_score_trend_detail_summary_extrema_comparison_detail_summary_extrema_report_summary_detail_summary(
    history: list[Mapping[str, Any]],
    lower_bound: int,
    upper_bound: int,
) -> dict[str, Any]:
    """Build a compact summary from per-metric report-summary details."""

    report = (
        build_quality_score_trend_detail_summary_extrema_comparison_detail_summary_extrema_report_summary_detail(
            history,
            lower_bound,
            upper_bound,
        )
    )

    metric_details = report["metric_details"]

    improving_metrics = tuple(
        item["metric"]
        for item in metric_details
        if item["is_improving"]
    )
    declining_metrics = tuple(
        item["metric"]
        for item in metric_details
        if item["is_declining"]
    )
    flat_metrics = tuple(
        item["metric"]
        for item in metric_details
        if item["is_flat"]
    )

    positive_change_metrics = tuple(
        item["metric"]
        for item in metric_details
        if item["change"] > 0
    )
    negative_change_metrics = tuple(
        item["metric"]
        for item in metric_details
        if item["change"] < 0
    )

    status_counts: dict[str, int] = {}

    for item in metric_details:
        status = item["status"]
        direction = status["direction"]

        status_counts[direction] = status_counts.get(direction, 0) + 1

    return {
        "snapshot_count": report["snapshot_count"],
        "lower_bound": report["lower_bound"],
        "upper_bound": report["upper_bound"],
        "quality_trend": report["quality_trend"],
        "stability_trend": report["stability_trend"],
        "consistency_trend": report["consistency_trend"],
        "quality_change": report["quality_change"],
        "stability_change": report["stability_change"],
        "consistency_change": report["consistency_change"],
        "largest_change_metric": report["largest_change_metric"],
        "largest_change": report["largest_change"],
        "overall_change_direction": report["overall_change_direction"],
        "widest_metric": report["widest_metric"],
        "widest_range": report["widest_range"],
        "improving_metrics": improving_metrics,
        "declining_metrics": declining_metrics,
        "flat_metrics": flat_metrics,
        "positive_change_metrics": positive_change_metrics,
        "negative_change_metrics": negative_change_metrics,
        "improving_metric_count": len(improving_metrics),
        "declining_metric_count": len(declining_metrics),
        "flat_metric_count": len(flat_metrics),
        "positive_change_metric_count": len(positive_change_metrics),
        "negative_change_metric_count": len(negative_change_metrics),
        "metric_count": len(metric_details),
        "status_counts": status_counts,
        "metric_details": metric_details,
        "details": report["details"],
    }
