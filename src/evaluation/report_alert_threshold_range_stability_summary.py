...
def calculate_unstable_count(
    ...
    return len(history) - calculate_stable_count(
        history,
        lower_bound,
        upper_bound,
    )
...
def calculate_instability_ratio(
    history: list[Mapping[str, Any]],
    lower_bound: int,
    upper_bound: int,
) -> float:
    ...
    return calculate_unstable_count(
        history,
        lower_bound,
        upper_bound,
    ) / len(history)
...
def build_stability_summary(
    ...
    """Build a complete range-stability summary."""
    _validate_history(history)
    _validate_bounds(lower_bound, upper_bound)
    ...
    return {
        ...
        "instability_percentage": instability_ratio * 100.0,
    }
